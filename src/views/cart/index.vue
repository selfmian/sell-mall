...
const computedCheckedAll = () => {
  ...
  // 计算多少品牌被选中
  const checkedCount = list.data.reduce(
    (prev, cur, index, arr) => {
      if (cur.checked) {
        return prev + 1;
      }
  ...
  // 计算有多少品牌被半选
  const isIndeterminate = list.data.reduce(
    (prev, cur, index, arr) => {
      if (cur.checked) {
        return prev + 1;
      }
  ...
  data.isIndeterminate = (checkedCount > 0 && checkedCount < list.data.length) || (isIndeterminate > 0 && isIndeterminate < list.data.length);
